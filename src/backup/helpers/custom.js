...
module.exports = {
  generateSinglePass: async (
    ...
    try {
      ...
      let pickupobj = getRouteStops.stops.find(
        ({ id }) => id.toString() === pickup_stop_id.toString()
      );
      let dropobj = getRouteStops.stops.find(
        ({ id }) => id.toString() === drop_stop_id.toString()
      );
      ...
      if (pickupobj && dropobj) {
        ...
        let drop_name =
          dropobj.location.title != "" ? dropobj.location.title : dropobj.title;
        ...
        const pass_total_fare = (
          final_total_fare.total * pass_no_of_rides
        ).toString();
        ...
        for (let i = 1; i <= total_pass_rides; i++) {
          ...
          const obj = {
            ...
            sub_total: (
              final_total_fare.total - final_total_fare.tax
            ).toString(),
